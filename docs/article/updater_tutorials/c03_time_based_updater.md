...
之前我们提到了有起始和终止的动画，以及==无限播放==的动画。这一节我们将探讨后者如何来编写。
...
很明显，当更新函数的参数中如果有一个名字为 dt ，那么就会执行下面的步骤，即将==基于时间的更新函数==添加到成员列表中。接下来就只需要接着上一节的分析，在每次 _动画播放_ 的时候，其中的每一帧都会调用这一函数，对物件进行更新了。

然而我们好像忽略了 `wait` 这个方法。我们发现，基于时间的更新不管是遇到 `play` 还是 `wait`，都会导致它的更新。
...
如果你运行了上面的例子，就会发现 10 秒钟的等待时间，秒针刚好就走了 10 个刻度。这看似是个巧合，但是我们可以使用 print 来 debug 呀。如果尝试在更新函数中加入一句 `print(dt)`，那么会打印一堆小数。这些小数大多都接近 0.0166 ，这是因为 dt 就是==帧率的倒数==。也就是说，每一秒钟，dt 在这一区间段的总和就是 1 。
...
> [!example] 参考解答
> 其实只需要改第 6 行就可以了。当发射角的变化是非线性的，其实就可以达到这样的效果。
> ```python {6}
> class Danmaku01(Scene):
>     def construct(self):
>         danmaku = VGroup()
