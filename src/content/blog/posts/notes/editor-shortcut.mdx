...
在 macOS 中，<kbd>⌥</kbd> <kbd>A</kbd> 会得到一些希腊字母，其中 <kbd>A</kbd> 指代的是 ASCII 按键的任意一个，包括字母，数字，字符符号等，很怪。当然，在 iTerm2 中，我们可以找到 `/ico vscode-icons:file-type-config;Preferences > Profiles > Keys > General` 配置中，有 Option Key 的选项（分别是左右两个）
...
在 `/ico vscode-icons:file-type-config;Preferences > Profiles > Keys > Key Mappings` 中，将 Presets 更改为 ==Natural Text Editing==.
