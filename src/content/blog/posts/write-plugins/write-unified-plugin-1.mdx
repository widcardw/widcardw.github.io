---
title: 如何编写 unified 插件 (1) remark
pubDate: 2023-07-14
...
tags:
  ...
  - unified
  - mdx
description: 官方文档给了示例，只是需要我们非常仔细地去找才能找到，同时这类插件本身就比较难写，所以还是值得整理一下的。本篇进行一些基本介绍和简单 remark 插件编写的引入。
---
...
而 unified 组织尝试开发了一套通用的 markdown 工具，能够让 markdown，语法分析树，html 代码相互无缝转换，其中的桥梁就是==语法分析树==。
...
目前 remark 插件的编写方式似乎主要有两种。一种是由 wooorm 主导的 JSDoc，`enter` & `exit` 写法，这种方式诞生于其生态中的 [micromark](https://github.com/micromark/micromark) 模块，将 markdown 转换为 tokens，然后经过 mdast-util 转换为 mdast 中的节点。

另一种是写起来更方便的 `visit` 写法。仅仅通过 `unist-util-visit` 这个模块中的 `visit` 方法，在遍历语法树的同时，更改树上的节点，从而影响下游的生成结果。该部分来源于 unifiedjs 官网给出的一个[教程](https://unifiedjs.com/learn/guide/create-a-plugin/)。

rehype 插件的编写方式主要就是通过 `visit` 方法遍历语法树，对特殊节点进行特殊处理。

## remark 插件

本篇先讨论如何使用 `visit` 方式编写 remark 插件，后续将会补充如何使用 micromark 编写插件。

这里给出一个参考：[`@widcardw/remark-callouts`](https://github.com/widcardw/remark-callouts)，是一个比较标准的，纯粹使用 `visit` 方法来编写的 remark 插件。

### 插件主体

我发现，可以用一个比较好的模板，只需编写 `visitBlock` 方法即可完成节点的修改了。只需将这个函数导出，即可作为一个插件。
...
这样的一个节点对应的就是包含 `class` 为 `color-red` 的 `span` 元素，文本内容是 `abc`。这样的 JS 对象与 HTML 中的 DOM 可以相互转换。接着，利用上述 `splice` 方法，将新的节点插入到相应的位置，即可完成对节点树的修改。

通过上述的流程，我们可以在一个 `/ico vscode-icons:file-type-typescript-official;index.ts` 文件中，快速地实现一个插件的功能。
